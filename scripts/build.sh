...
echo -e "${BLUE}🦉 Pwnagent Build Script${NC}"
echo "================================"
...
BINARY_NAME="pwnagent-${VERSION}-${OS_NAME}-${ARCH_NAME}"

if [ "$OS_NAME" = "windows" ]; then
    if [ ! -f "dist/pwnagent.exe" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    BINARY_NAME="${BINARY_NAME}.exe"
    cp "dist/pwnagent.exe" "$RELEASE_DIR/$BINARY_NAME"
    ...
    if [ ! -f "dist/pwnagent" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    cp "dist/pwnagent" "$RELEASE_DIR/$BINARY_NAME"
