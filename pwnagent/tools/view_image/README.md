...
- **Implementation:** `agents.sandbox.capabilities.tools.view_image.ViewImageTool`
  (upstream `agents` SDK)
- **Wired in:** `pwnagent/agents/factory.py` — added per-run via the SDK
  `Filesystem` capability.
- **Pwnagent defaults:** screenshots default to
...
- **Skill:** screenshot workflow lives in `pwnagent/skills/tooling/agent_browser.md`.
- **Recovery:** vision-not-supported model rejections are auto-recovered
  via `pwnagent.core.sessions.strip_all_images_from_session`, invoked from
  `pwnagent/core/execution.py`.
