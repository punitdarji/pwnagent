"""Build SandboxAgents for root + child Pwnagent runs."""
...
from pwnagent.tools.agents_graph.tools import (
    agent_finish,
    create_agent,
...
from pwnagent.tools.finish.tool import finish_scan
from pwnagent.tools.load_skill.tool import load_skill
from pwnagent.tools.notes.tools import (
    create_note,
    delete_note,
    get_note,
    list_notes,
...
from pwnagent.tools.proxy.tools import (
    list_requests,
    list_sitemap,
    repeat_request,
    scope_rules,
...
from pwnagent.tools.reporting.tool import create_dependency_report, create_vulnerability_report
from pwnagent.tools.thinking.tool import think
from pwnagent.tools.todo.tools import (
    create_todo,
    delete_todo,
    list_todos,
    mark_todo_done,
...
from pwnagent.tools.cve_scanner.tool import find_cve_nuclei_templates, run_cve_nuclei_scan
from pwnagent.tools.web_search.tool import web_search
...
# ``pwnagent.runtime.backends.register_backend``: register before the first
# ``build_pwnagent_agent`` call and every agent (root + children) gets them.
