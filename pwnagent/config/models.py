...
if TYPE_CHECKING:
    ...
    from pwnagent.config.settings import Settings


class PwnagentProvider(MultiProvider):
...
def configure_sdk_model_defaults(settings: Settings) -> None:
    """Apply Pwnagent config to SDK-native defaults."""
    llm = settings.llm
...
def _configure_litellm_compatibility() -> None:
    ...
    # Pwnagent uses LiteLLM's success callback to capture provider-reported cost.
...
_OPENROUTER_ATTRIBUTION_HEADERS = {
    "HTTP-Referer": "https://pwnagent.ai",
    "X-Title": "Pwnagent",
...
def _register_litellm_cost_callback() -> None:
    ...
    from pwnagent.report.state import litellm_cost_callback
