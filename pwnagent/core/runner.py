"""Top-level Pwnagent scan runner."""
...
from pwnagent.agents.factory import build_pwnagent_agent, make_child_factory
from pwnagent.agents.prompt import render_system_prompt
...
from pwnagent.core.execution import (
    respawn_subagents,
    run_agent_loop,
)
from pwnagent.core.execution import (
...
from pwnagent.core.hooks import BudgetExceededError, ReportUsageHooks
from pwnagent.core.inputs import (
    DEFAULT_MAX_TURNS,
    build_root_task,
    build_scope_context,
    make_model_settings,
...
from pwnagent.core.sessions import open_agent_session
from pwnagent.runtime import session_manager
from pwnagent.telemetry.logging import set_scan_id, setup_scan_logging
...
async def run_pwnagent_scan(
    *,
    ...
    """Run or resume one Pwnagent scan against a sandbox.
    ...
    logger.info(
        "%s Pwnagent scan %s (image=%s, max_turns=%d, interactive=%s, run_dir=%s)",
    ...
    if not resolved_model:
        raise RuntimeError(
            "No LLM model configured. Set PWNAGENT_LLM env or pass model= to run_pwnagent_scan().",
        )
    ...
    from pwnagent.tools.notes.tools import hydrate_notes_from_disk
    from pwnagent.tools.todo.tools import hydrate_todos_from_disk
    ...
    try:
        ...
        run_config = RunConfig(
            ...
            model_provider=PwnagentProvider(),
        ...
        root_agent = build_pwnagent_agent(
            name="pwnagent",
            skills=skills,
            is_root=True,
            scan_mode=scan_mode,
        ...
        logger.warning(
            ...
            "Resume with 'pwnagent --resume %s' once the limit clears.",
            scan_id,
            exc,
            scan_id,
        )
        ...
        logger.exception("Pwnagent scan %s failed", scan_id)
        if root_id is not None:
            await coordinator.cancel_descendants(root_id)
            with contextlib.suppress(Exception):
        ...
        logger.info("Pwnagent scan %s done", scan_id)
        teardown_logging()
