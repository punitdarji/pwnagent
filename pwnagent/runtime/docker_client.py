"""PwnagentDockerSandboxClient — preserves the image's ENTRYPOINT and adds
...
class PwnagentDockerSandboxClient(DockerSandboxClient):
    ...
    pwnagent_bind_mounts: list[dict[str, Any]] | None = None
    ...
    async def _create_container(
        ...
        *,
        ...
        # Pwnagent delta from the SDK body: drop ``entrypoint`` override and
        ...
        # Pwnagent injections — append, don't overwrite, so FUSE/SYS_ADMIN survives.
        ...
        # Pwnagent injection: host bind mounts (e.g. large repos passed via --mount)
        # that bypass the SDK's file-by-file LocalDir copy.
        bind_mounts = getattr(self, "pwnagent_bind_mounts", ())
        if bind_mounts:
