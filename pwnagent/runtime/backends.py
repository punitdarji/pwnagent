"""Sandbox backend registry — selected via PWNAGENT_RUNTIME_BACKEND (default: docker)."""
...
def _start_attempts() -> int:
    raw = os.environ.get("PWNAGENT_SANDBOX_START_ATTEMPTS")
    if raw is None:
    ...
    try:
        ...
        logger.warning(
            "Invalid PWNAGENT_SANDBOX_START_ATTEMPTS=%r; using %d",
            raw,
    ...
    if attempts < 1:
        logger.warning(
            "PWNAGENT_SANDBOX_START_ATTEMPTS must be positive; using %d",
            _DEFAULT_START_ATTEMPTS,
        )
        return _DEFAULT_START_ATTEMPTS
    return attempts
...
async def _docker_backend(
    *,
    ...
    Uses :class:`PwnagentDockerSandboxClient` to inject NET_ADMIN /
    ...
    would call it too, but Pwnagent manages session lifetime explicitly via
    ...
    from pwnagent.runtime.docker_client import PwnagentDockerSandboxClient

    client = PwnagentDockerSandboxClient(docker.from_env())
    client.pwnagent_bind_mounts = bind_mounts or []
...
def get_backend(name: str) -> SandboxBackend:
    ...
    if backend is None:
        ...
        raise ValueError(
            f"Unknown PWNAGENT_RUNTIME_BACKEND: {name!r} (supported: {supported})",
        )
