...
from pwnagent.config.models import is_recommended_or_frontier_model
from pwnagent.core.hooks import BudgetExceededError
from pwnagent.core.runner import run_pwnagent_scan
from pwnagent.interface.tui.live_view import TuiLiveView
from pwnagent.interface.tui.messages import send_user_message_to_agent
from pwnagent.interface.tui.renderers import render_tool_widget
from pwnagent.interface.tui.renderers.agent_message_renderer import AgentMessageRenderer
from pwnagent.interface.tui.renderers.user_message_renderer import UserMessageRenderer
from pwnagent.interface.utils import build_tui_stats_text
from pwnagent.report.state import ReportState, set_global_report_state
from pwnagent.runtime import session_manager
...
def get_package_version() -> str:
    try:
        return pkg_version("pwnagent-agent")
...
class ChatTextArea(TextArea):  # type: ignore[misc]
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._app_reference: PwnagentTUIApp | None = None

    def set_app_reference(self, app: "PwnagentTUIApp") -> None:
...
class SplashScreen(Static):  # type: ignore[misc]
    ...
    def _build_url_text(self) -> Text:
        return Text("pwnagent.ai", style=Style(color=self.PRIMARY_GREEN, bold=True))

    def _build_welcome_text(self) -> Text:
        text = Text("Welcome to ", style=Style(color="white", bold=True))
        text.append("Pwnagent", style=Style(color=self.PRIMARY_GREEN, bold=True))
    ...
    def _build_start_line_text(self, phase: int) -> Text:
        full_text = "Starting Pwnagent Agent"
...
class HelpScreen(ModalScreen):  # type: ignore[misc]
    def compose(self) -> ComposeResult:
        yield Grid(
            Label("Pwnagent Help", id="help_title"),
            Label(
                "F1        Help\nCtrl+Q/C  Quit\nESC       Stop Agent\n"
...
class QuitScreen(ModalScreen):  # type: ignore[misc]
    def compose(self) -> ComposeResult:
        yield Grid(
            Label("Quit Pwnagent?", id="quit_title"),
            Grid(
...
class PwnagentTUIApp(App):  # type: ignore[misc]
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        from pwnagent.core.agents import AgentCoordinator
    ...
    def _merge_renderables(renderables: list[Any]) -> Text:
        ...
        for i, item in enumerate(renderables):
            if i > 0:
                combined.append("\n")
            PwnagentTUIApp._append_renderable(combined, item)
        return PwnagentTUIApp._sanitize_text(combined)
    ...
    def _append_renderable(combined: Text, item: Any) -> None:
        ...
        if isinstance(item, Text):
            combined.append_text(PwnagentTUIApp._sanitize_text(item))
        elif isinstance(item, Group):
            for j, sub in enumerate(item.renderables):
                if j > 0:
                    combined.append("\n")
                PwnagentTUIApp._append_renderable(combined, sub)
        else:
            inner = getattr(item, "content", None) or getattr(item, "renderable", None)
            if inner is not None:
                PwnagentTUIApp._append_renderable(combined, inner)
            else:
            ...
            try:
                ...
                try:
                    if not self._scan_stop_event.is_set():
                        image = load_settings().runtime.image or "pwnagent-sandbox:latest"
    ...
    def _clean_copied_text(text: str) -> str:
        ...
        for line in lines:
            ...
            if stripped in PwnagentTUIApp._DECORATIVE_LINES:
                continue
            if stripped and all(c == "─" for c in stripped):
                continue
            out = line
            for prefix in PwnagentTUIApp._ICON_PREFIXES:
