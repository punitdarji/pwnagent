...
from pwnagent.config import (
    apply_config_override,
    load_settings,
    persist_current,
)
from pwnagent.config.models import (
...
from pwnagent.core.paths import run_dir_for, runtime_state_dir
from pwnagent.interface.cli import run_cli
from pwnagent.interface.tui import run_tui
from pwnagent.interface.utils import (
...
from pwnagent.report.state import get_global_report_state
from pwnagent.report.writer import read_run_record, write_run_record
from pwnagent.telemetry import posthog, scarf
from pwnagent.telemetry.logging import configure_dependency_logging
...
BEDROCK_EXTRA_HINT = (
    'Bedrock support is optional. Install it with: pipx install "pwnagent-agent[bedrock]"'
)
VERTEX_MODEL_MARKER = "vertex"
VERTEX_MISSING_MODULE_ERROR = "No module named 'google"
VERTEX_EXTRA_HINT = (
    'Vertex AI support is optional. Install it with: pipx install "pwnagent-agent[vertex]"'
)
...
def validate_environment() -> None:
    ...
    if not settings.llm.model:
        missing_required_vars.append("PWNAGENT_LLM")
    ...
    if missing_required_vars:
        ...
        for var in missing_required_vars:
            if var == "PWNAGENT_LLM":
                ...
                error_text.append("PWNAGENT_LLM", style="bold cyan")
                error_text.append(
        ...
        if missing_optional_vars:
            ...
            for var in missing_optional_vars:
                ...
                elif var == "PWNAGENT_REASONING_EFFORT":
                    error_text.append("• ", style="white")
                    error_text.append("PWNAGENT_REASONING_EFFORT", style="bold cyan")
                    error_text.append(
        ...
        error_text.append("\nExample setup:\n", style="white")
        error_text.append("export PWNAGENT_LLM='openai/gpt-5.4'\n", style="dim white")
        ...
        if missing_optional_vars:
            for var in missing_optional_vars:
                if var == "LLM_API_KEY":
                    ...
                    error_text.append(
                        "export PWNAGENT_REASONING_EFFORT='high'\n",
                        style="dim white",
                    )

        panel = Panel(
            error_text,
            title="[bold white]PWNAGENT",
            title_align="left",
            border_style="red",
            padding=(1, 2),
        )
...
async def warm_up_llm(show_model_warning: bool = True) -> None:
    ...
    try:
        ...
        if show_model_warning and raw_model and not is_recommended_or_frontier_model(raw_model):
            ...
            warn_text.append(f"'{raw_model}'", style="bold cyan")
            warn_text.append(
                " is not a recommended frontier model for Pwnagent.\nSecurity scans work best with:\n",
                style="white",
            )
        ...
        model = PwnagentProvider().get_model(raw_model)
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Pwnagent Multi-Agent Cybersecurity Penetration Testing Tool",
...
Examples:
  ...
  pwnagent --target https://github.com/user/repo
  pwnagent --target git@github.com:user/repo.git
  ...
  pwnagent --target https://github.com/user/repo --target https://example.com
  pwnagent --target ./my-project --target https://staging.example.com --target https://prod.example.com

  # Targets from a file, one target per non-empty, non-comment line
  pwnagent --target-list ./targets.txt

  # Custom instructions (inline)
  pwnagent --target example.com --instruction "Focus on authentication vulnerabilities"

  # Custom instructions (from file)
  pwnagent --target example.com --instruction-file ./instructions.txt
  pwnagent --target https://app.com --instruction-file /path/to/detailed_instructions.md
        """,
    )
    ...
    parser.add_argument(
        ...
        "--version",
        action="version",
        version=f"pwnagent {get_version()}",
    )
    ...
    parser.add_argument(
        "--config",
        type=str,
        help="Path to a custom config file (JSON) to use instead of ~/.pwnagent/cli-config.json",
    )
    ...
    parser.add_argument(
        ...
        metavar="RUN_NAME",
        help=(
            "Resume a prior scan by its run name (the dir under ./pwnagent_runs/). "
    ...
    if args.resume:
        ...
        if oversized:
            ...
            parser.error(
                ...
                "(set PWNAGENT_MAX_LOCAL_COPY_MB to change it). Re-run with "
...
def display_completion_message(args: argparse.Namespace, results_path: Path) -> None:
    ...
    if not scan_completed:
        ...
        resume_text.append("  ")
        resume_text.append(f"pwnagent --resume {args.run_name}", style="#22c55e")
        panel_parts.extend(["\n", resume_text])
