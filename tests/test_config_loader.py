"""Tests for pwnagent.config.loader: JSON overrides, alias resolution, persistence."""
...
from pwnagent.config import loader
...
_LLM_ENV_KEYS = [
    ...
    "LITELLM_BASE_URL",
    "OLLAMA_API_BASE",
    "PWNAGENT_REASONING_EFFORT",
    "PWNAGENT_FORCE_REQUIRED_TOOL_CHOICE",
    "LLM_TIMEOUT",
    "PERPLEXITY_API_KEY",
    # RuntimeSettings
    "PWNAGENT_IMAGE",
    "PWNAGENT_RUNTIME_BACKEND",
    "PWNAGENT_MAX_LOCAL_COPY_MB",
    # TelemetrySettings
    "PWNAGENT_TELEMETRY",
]
...
def test_read_json_overrides_maps_to_nested_settings(tmp_path: Path) -> None:
    path = tmp_path / "cli-config.json"
    path.write_text(
        json.dumps({"env": {"PWNAGENT_LLM": "my-model", "PERPLEXITY_API_KEY": "pk"}}),
        encoding="utf-8",
    )
    assert loader._read_json_overrides(path) == {
        "llm": {"model": "my-model"},
...
def test_read_json_overrides_skips_keys_already_in_environ(
    ...
    monkeypatch.setenv("PWNAGENT_LLM", "from-env")
    path = tmp_path / "cli-config.json"
    path.write_text(json.dumps({"env": {"PWNAGENT_LLM": "from-file"}}), encoding="utf-8")
...
def test_apply_override_and_load_settings_round_trip(tmp_path: Path) -> None:
    path = tmp_path / "cli-config.json"
    path.write_text(
        json.dumps({"env": {"PWNAGENT_LLM": "round-trip-model", "PERPLEXITY_API_KEY": "pk"}}),
        encoding="utf-8",
    )
...
def test_apply_config_override_invalidates_cache(tmp_path: Path) -> None:
    ...
    first.write_text(json.dumps({"env": {"PWNAGENT_LLM": "first-model"}}), encoding="utf-8")
    second = tmp_path / "second.json"
    second.write_text(json.dumps({"env": {"PWNAGENT_LLM": "second-model"}}), encoding="utf-8")
...
def test_persist_current_writes_env_block(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("PWNAGENT_LLM", "persisted-model")
    ...
    assert json.loads(target.read_text(encoding="utf-8")) == {
        "env": {"PWNAGENT_LLM": "persisted-model"}
    }
