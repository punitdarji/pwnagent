...
cli_main: Any = importlib.import_module("pwnagent.interface.main")
...
def test_parse_arguments_accepts_target_list_file(
    ...
    monkeypatch.setattr(sys, "argv", ["pwnagent", "--target-list", str(target_list), "-n"])
...
def test_parse_arguments_combines_target_and_target_list(
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["pwnagent", "-t", "https://test1.com/", "--target-list", str(target_list)],
    )
...
def test_parse_arguments_rejects_resume_with_target_list(
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["pwnagent", "--resume", "old-run", "--target-list", str(target_list)],
    )
