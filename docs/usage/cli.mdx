---
title: "CLI Reference"
description: "Command-line options for Pwnagent"
---

## Basic Usage

```bash
pwnagent (--target <target> | --target-list <path> | --mount <path>) [options]
```
...
<ParamField path="--mount" type="string">
  ...
  Pwnagent copies local `--target` directories into the sandbox one file at a time, which stalls on very large trees. When a local target exceeds the copy limit (see `PWNAGENT_MAX_LOCAL_COPY_MB`, default 1024 MB) Pwnagent exits early and asks you to re-run with `--mount`.
...
<ParamField path="--config" type="string">
  Path to a custom config file (JSON) to use instead of `~/.pwnagent/cli-config.json`.
</ParamField>
...
<ParamField path="--max-budget-usd" type="number">
  ...
  **Limitations**
  ...
  - For LiteLLM-routed models, Pwnagent enables streaming success callbacks to
...
pwnagent --target https://app.com --instruction "Use credentials: user:pass"

# Focused testing
pwnagent --target api.example.com --instruction "Focus on IDOR and auth bypass"

# CI/CD mode
pwnagent -n --target ./ --scan-mode quick
...
pwnagent -t https://github.com/org/app -t https://staging.example.com
