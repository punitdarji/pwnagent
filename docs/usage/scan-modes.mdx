...
Pwnagent offers three scan modes to balance speed and thoroughness.

## Quick

```bash
pwnagent --target ./app --scan-mode quick
```
...
```bash
pwnagent --target ./app --scan-mode standard
```
...
```bash
pwnagent --target ./app --scan-mode deep
```
