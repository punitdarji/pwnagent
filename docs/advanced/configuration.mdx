---
title: "Configuration"
description: "Environment variables for Pwnagent"
---

Configure Pwnagent using environment variables or a config file.

## LLM Configuration

<ParamField path="PWNAGENT_LLM" type="string" required>
...
<ParamField path="PWNAGENT_LLM_MAX_RETRIES" default="5" type="integer">
  Maximum number of retries for LLM API calls on transient failures.
</ParamField>

<ParamField path="PWNAGENT_REASONING_EFFORT" default="high" type="string">
...
<ParamField path="PWNAGENT_MEMORY_COMPRESSOR_TIMEOUT" default="30" type="integer">
...
<ParamField path="PWNAGENT_TELEMETRY" default="1" type="string">
...
<ParamField path="TRACELOOP_BASE_URL" type="string">
  OTLP/Traceloop base URL for remote OpenTelemetry export. If unset, Pwnagent keeps traces local only.
</ParamField>
...
When remote OTEL vars are not set, Pwnagent still writes complete run telemetry locally to:

```bash
pwnagent_runs/<run_name>/events.jsonl
```

When remote vars are set, Pwnagent dual-writes telemetry to both local JSONL and the remote OTEL endpoint.

## Docker Configuration

<ParamField path="PWNAGENT_IMAGE" default="ghcr.io/punitdarji/pwnagent-sandbox:1.0.0" type="string">
...
<ParamField path="PWNAGENT_RUNTIME_BACKEND" default="docker" type="string">
  Runtime backend for the sandbox environment.
</ParamField>

<ParamField path="PWNAGENT_MAX_LOCAL_COPY_MB" default="1024" type="integer">
  Maximum size (in MB) of a local directory target that Pwnagent will copy into the sandbox file-by-file. Larger targets exit early with a suggestion to use `--mount` instead. Set to `0` to disable the check.
</ParamField>

## Sandbox Configuration

<ParamField path="PWNAGENT_SANDBOX_EXECUTION_TIMEOUT" default="120" type="integer">
  Maximum execution time in seconds for sandbox operations.
</ParamField>

<ParamField path="PWNAGENT_SANDBOX_CONNECT_TIMEOUT" default="10" type="integer">
  Timeout in seconds for connecting to the sandbox container.
</ParamField>

## Config File

Pwnagent stores configuration in `~/.pwnagent/cli-config.json`. You can also specify a custom config file:

```bash
pwnagent --target ./app --config /path/to/config.json
```
...
{
  "env": {
    "PWNAGENT_LLM": "openai/gpt-5.4",
    "LLM_API_KEY": "sk-...",
    "PWNAGENT_REASONING_EFFORT": "high"
  }
}
```
...
export PWNAGENT_SANDBOX_EXECUTION_TIMEOUT="300"
