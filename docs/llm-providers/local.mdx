---
title: "Local Models"
description: "Run Pwnagent with self-hosted LLMs for privacy and air-gapped testing"
---

Running Pwnagent with local models allows for completely offline, privacy-first security assessments. Data never leaves your machine, making this ideal for sensitive internal networks or air-gapped environments.
...
<Warning>
**Compatibility Note**: Pwnagent relies on advanced agentic capabilities (tool use, multi-step planning, self-correction). Most local models, especially those under 70B parameters, struggle with these complex tasks.
...
2. Pull a high-performance model:
   ...
   ```
3. Configure Pwnagent:
   ```bash
   export PWNAGENT_LLM="ollama/qwen3-vl"
...
```bash
export PWNAGENT_LLM="openai/local-model"
