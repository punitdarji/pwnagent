...
# hostname (rather than an IP literal), set PWNAGENT_CAIDO_ALLOWED_DOMAINS to a
...
if [ -n "${PWNAGENT_CAIDO_ALLOWED_DOMAINS:-}" ]; then
  IFS=',' read -ra _caido_domains <<< "${PWNAGENT_CAIDO_ALLOWED_DOMAINS}"
